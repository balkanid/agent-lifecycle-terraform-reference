...
| `IamRolesUnderLifecyclePath` | `aws_iam_role` + `aws_iam_role_policy` under `/balkanid-agent-lifecycle/` — scoped with **`iam:ResourcePath`** (required for `GetRole` on path-prefixed roles; `role/balkanid-agent-lifecycle/*` alone is not enough in all API checks) |
